...
AI는 `AiEngine`에서 성격별 행동을 선택하고 `GameEngine`이 선택 결과를 GameState와 로그에 적용한다. AI 판단과 상태 변경을 분리해 장기 시뮬레이션과 밸런스 검증이 가능하다. AI 군사 행동도 `BattleEngine`을 호출하며, Flame은 플레이어 전투와 동일하게 전투 화면을 렌더링할 수 있는 경계를 유지한다.
```
