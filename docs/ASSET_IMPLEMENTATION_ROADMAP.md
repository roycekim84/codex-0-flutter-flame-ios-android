...
- [ ] P0 에셋이 생성되고 `assets`에 저장됐다.
- [ ] P0 에셋이 `AssetRepository` key와 연결됐다.
- [ ] 참고 이미지 17개 화면에 전용 이미지 요소가 최소 1개 이상 있다.
- [ ] 장수·세력·지역 이미지가 데이터 ID와 일치한다.
- [ ] 전투 지형·부대·이펙트가 Flame component로 연결됐다.
- [ ] 일반/선택/비활성/경고 상태 이미지가 구분된다.
- [ ] 390×844와 Web에서 레이아웃이 무너지지 않는다.
- [ ] `flutter test`, `flutter analyze`, `flutter build web --release`가 통과한다.
- [ ] GitHub Pages에서 배포된 이미지가 확인된다.

## 8. 다음 실행 단계

다음 구현은 **Stage 1 — 공통 UI 크롬**부터 시작한다. 먼저 패널·버튼·탭·자원 아이콘을 생성하고 공통 위젯으로 연결한 뒤, 장수·지도·전투 에셋으로 확장한다.
